...
let init_value = 666 (* �viter 0 pour v�rifier visuellement les init. *)
...
(* exceptions pouvant �tre lev�es durant l'ex�cution *)
...
(* diverses fonctions d'acc�s et de modification aux piles et aux tas *)
...
(* ex�cution jusqu'� [Stop] ou jusqu'� la premi�re exception *)
	
let run () = try while true do step () done with Exit -> ()

(* �criture/lecture de l'�tat de la machine sur le disque *)
